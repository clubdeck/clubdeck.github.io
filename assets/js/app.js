...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  if (window.web3) {
    ...
    log("Attempting to retrieve user accounts")
...
const updateConnectStatus = async () => {
  ...
  if (!MetamaskOnboarding.isMetaMaskInstalled()) {
    onboardButton.innerText = "Install the MetaMask Wallet"
    ...
    onboardButton.innerText = `✔ ${accounts[0].slice(0,4)}...${accounts[0].slice(-4)}`
    ...
    onboardButton.innerText = "Connect your MetaMask wallet"
    // HIDE SPINNER
    spinner.classList.add('hidden')
    notConnected.classList.remove('hidden')
    notConnected.classList.add('show-not-connected')
...
async function loadInfo() {
  ...
  if (publicMintActive) {
    ...
    try {
      ...
      const whitelisted = await contract.methods.isWhitelisted(window.address, merkleData).call()
      if(!whitelisted) {
...
function getMerkleProofs(address) {
  let result = merkleProofs[address]
  if ('undefined' === typeof(result)) result = merkleProofs[address.toLowerCase()]
  if ('undefined' === typeof(result)) result = merkleProofs[address.toUpperCase()]
  return result
}